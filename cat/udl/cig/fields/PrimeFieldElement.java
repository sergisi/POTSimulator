...
public class PrimeFieldElement implements RingElement {
	...
	/**
	 * A BigInteger that encapsulates the value of this <i>Prime Field Element</i>.
	 */
	 ...
	 * uninitialized. That is {@code this.F = null} and {@code this.k = null}. This
	 * constructor does not make a deep compy of \(F\).
	 *
	 * @param F the <i>PrimeField</i> to which {@code this} <i>PrimeFieldElement</i>
	 *          will belong.
	...
	public PrimeFieldElement(final PrimeField F, final BigInteger k) {
		this.F = F;
		if (k.compareTo(F.getSize()) >= 0 || k.compareTo(BigInteger.ZERO) < 0) {
	...
	protected PrimeFieldElement(final PrimeField F, final BigInteger k, final boolean safe) {
		this.F = F;
		this.k = k;
	}
	...
	@Override
	public PrimeFieldElement add(final RingElement q) throws IncorrectRingElementException {
		if (belongsToSameGroup(q)) {
			...
			throw new IncorrectRingElementException(
					"RingElement q is not a " + "correct instance of PrimeFieldElement");
		}
	}

	@Override
	public PrimeFieldElement subtract(final RingElement q) throws IncorrectRingElementException {
		if (belongsToSameGroup(q)) {
	...
	@Override
	public PrimeFieldElement multiply(final GroupElement q) throws IncorrectRingElementException {
		if (belongsToSameGroup(q)) {
	...
	@Override
	public PrimeFieldElement divide(final GroupElement q) throws IncorrectRingElementException {
		if (belongsToSameGroup(q)) {
			BigInteger val = k.multiply((q.getIntValue()).modInverse(F.getSize()));
			val = val.mod(F.getSize());
			return new PrimeFieldElement(F, val, true);
		} else {
			throw new IncorrectRingElementException(
	...
	public ArrayList<RingElement> squareRoot() {
		...
		BigInteger b = new BigInteger(F.getSize().bitLength(), new Random());
		...
		BigInteger r = k.modPow(mod1.add(BigInteger.ONE).divide(new BigInteger("2")), F.getSize());

		for (int i = 1; i < e.intValue(); i++) {
			BigInteger exp = BigInteger.valueOf(2).pow(e.intValue() - i - 1);
			BigInteger d = (r.pow(2)).multiply(inverse).modPow(exp, F.getSize());

			if (d.mod(F.getSize()).compareTo(new BigInteger("-1").mod(F.getSize())) == 0) {
				r = r.multiply(c);
				r.mod(F.getSize());
			}
	 ...
	 */
	private int jacobiSymbol(final BigInteger k, final BigInteger m) throws IncorrectModuleException {
		...
		if (m.equals(ZERO)) {
			throw new IncorrectModuleException("Module m can not be equal to 0");
			/*
			 * Caldria mirar si m és un primer? Si pot no ser un primer, com comprovo que
			 * la primera crida es fa sobre un PrimeField?
			 */
		}
		...
		if (!(k1.equals(ONE))) {
			...
			s = s.multiply(new BigInteger(String.valueOf(jacobiSymbol(m1, k1))));
		}
		return s.intValue();
	}

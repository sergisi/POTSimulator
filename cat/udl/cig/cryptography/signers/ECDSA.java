...
public class ECDSA extends Signer {
	 ...
	 * <i>ECDSA</i> signer. The private key \( d \) is randomly chosen in the range
	 * \( [1, q] \), where \( q = ord(P) \) is the order of the generator point
	 * {@code P}.
	 */
	private final BigInteger privateKey;

	/**
	 * An <i>ECPoint</i> that represents the public key of {@code this} <i>ECDSA</i>
	 * signer. The public key \( Q \) is computed as \( Q = d \cdot P \)
	 */
	private final ECPoint publicKey;

	/**
	 * Creates an instance of <i>ECDSA</i> signer. This constructor also initializes
	 * the corresponding <i>ECDSAVerifier</i> with the public parameters.
	 *
	 * @param E the Elliptic Curve to which the message and the corresponding
...
//		if(generator.pow(E.getSize()).isInfinity() == TRUE){
//            ((GeneralECPoint)generator.setOrder(E.getSize());
//        }
	...
	public ECDSA(final ECPrimeOrderSubgroup E, ECPoint generator, ECPoint publicKey, BigInteger privateKey) {
		this.F = E;
		this.generator = generator;
		this.publicKey = publicKey;
		this.privateKey = privateKey;

		this.verifier = new ECDSAVerifier(generator, publicKey);
	...
	public Signature sign(final Object message) {
		...
		/*
		 * return new Signature(new PrimeFieldElement((PrimeField) F, r), new
		 * PrimeFieldElement((PrimeField) F, s));
		 */

//		PrimeField f = (PrimeField) F;
//		PrimeFieldElement rPrimeFieldElement = new PrimeFieldElement(f, r);
//		PrimeFieldElement sPrimeFieldElement = new PrimeFieldElement(f, s);

		Signature result = new Signature(r, s);
		return (result);
	}
}
